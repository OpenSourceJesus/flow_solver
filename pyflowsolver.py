...
def make_dir_vars(puzzle, start_var):
	...
	for i, j, char in explode(puzzle):
		...
		neighborBits = (dir_bit for (dir_bit, ni, nj)
						 in valid_neighbors(size, i, j))

		# OR them all together
		cellFlags = 0
		for neighborBit in neighborBits:
			cellFlags |= neighborBit
...
def show_solution(options, colors, decoded):
	...
	for char, color in colors.items():
		...
		do_color = do_color and char in ANSI_LOOKUP.keys()
	...
	for decoded_row in decoded:
		for (color, dir_type) in decoded_row:
			...
			if do_color:
				...
				if color_char in ANSI_LOOKUP.keys():
...
def pyflow_solver_main():
	...
	for filename in options.filenames:
		...
		if result_char not in stats.keys():
